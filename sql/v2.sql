...
update cw_region set gb_code_p = null, gb_code_c = null where province = '重庆' and level = 1;
...
(165, 'Weizhang', 'Xitong', 'stats_view', '', 1, 1, '违章API调用统计', '', '', 0);

ALTER TABLE `cw_endorsement`
ADD COLUMN `close_confirm`  int(1) NULL DEFAULT 0 COMMENT '违章处理确认：1 关闭违章；0 未关闭；' AFTER `office`,
ADD COLUMN `close_query_no`  varchar(32) NULL AFTER `close_confirm`,
ADD COLUMN `close_time`  int(11) NULL AFTER `close_query_no`;

INSERT INTO `cw_menu` (`parentid`, `app`, `model`, `action`, `data`, `type`, `status`, `name`, `icon`, `remark`, `listorder`) VALUES
(163, 'Weizhang', 'Jiaoyi', 'close_endorsements', '', 1, 1, '违章确认', '', '', 50);

ALTER TABLE `cw_message` ADD `order_id` INT(11) NULL COMMENT '订单id' ;
CREATE TABLE `cw_version` ( `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '自增长主键' , `url` VARCHAR(128) NOT NULL COMMENT '下载地址' , `version` VARCHAR(16) NOT NULL COMMENT '版本号' , `extra` VARCHAR(256) NOT NULL COMMENT '版本说明' , `is_must` INT(11) NOT NULL COMMENT '是否强制更新，0否 1是' , `update_time` INT(11) NOT NULL COMMENT '更新时间' , PRIMARY KEY (`id`) ) COMMENT = '版本更新';

INSERT INTO `cw_menu` (`parentid`, `app`, `model`, `action`, `data`, `type`, `status`, `name`, `icon`, `remark`, `listorder`) VALUES
(165, 'Admin_new', 'Xitong', 'yinhang', '', 1, 1, '银行管理(APP)', '', '', 0),
(165, 'Admin_new', 'Xitong', 'version', '', 1, 1, '版本管理(APP)', '', '', 0);

ALTER TABLE `cw_services` CHANGE `grade` `grade` DECIMAL(10,2) NULL COMMENT '评分';
ALTER TABLE `cw_services` ADD `status` INT(11) NULL COMMENT '是否接单状态：0是，1否' ;

ALTER TABLE `cw_expend` ADD `card_number` VARCHAR(64) CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '银行卡号' AFTER `expend_sn`;
ALTER TABLE `cw_expend` ADD `tixian_name` VARCHAR(32) NULL COMMENT '提现户名' AFTER `card_number`;

ALTER TABLE `cw_turn_order`
ADD COLUMN `so_type`  int(11) NULL DEFAULT 1 COMMENT '定价类型：1，静态定价；2，动态定价' AFTER `sod_id`;
ALTER TABLE `cw_turn_order`
...
`c_time`  int(11) NULL DEFAULT NULL COMMENT '获取时间' ,
PRIMARY KEY (`id`)
);

ALTER TABLE cw_expend ADD user_bank VARCHAR(128) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '开户银行' AFTER tixian_name;

ALTER TABLE `cw_car`
ADD UNIQUE INDEX `car_unique` (`license_number`, `frame_number`, `engine_number`) USING HASH ;

ALTER TABLE `cw_endorsement_jilu` CHANGE `city` `city` VARCHAR(64) CHARACTER SET gbk COLLATE gbk_chinese_ci NULL COMMENT '违章城市';